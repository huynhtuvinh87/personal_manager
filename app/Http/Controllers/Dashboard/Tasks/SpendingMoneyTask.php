...
class SpendingMoneyTask
{
    ...
    public function weekday()
    {
        ...
        $current_week = SpendingMoney::where('user_id', Auth::user()->id)
            ->where('date', '>=', $first_day_of_week)
            ->where('date', '<=', $last_day_of_week)
            ->sum("price");
        $last_week = SpendingMoney::where('user_id', Auth::user()->id)
    ...
    public function month()
    {
        ...
        $current_month = SpendingMoney::where('user_id', Auth::user()->id)
            ->where('date', '>=', $first_month)
            ->where('date', '<=', $last_month)
            ->sum("price");
        $last_month = SpendingMoney::where('user_id', Auth::user()->id)
    ...
    public function year()
    {
        ...
        $current_year = SpendingMoney::where('user_id', Auth::user()->id)
            ->where('date', '>=', $first_year)
            ->where('date', '<=', $last_year)
            ->sum("price");
        $last_year = SpendingMoney::where('user_id', Auth::user()->id)
